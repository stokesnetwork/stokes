...
type coinbaseManager struct {
	...
	// STOKES: Added halving interval for Bitcoin-style emission
...
//
// STOKES: Modified to implement Bitcoin-style halving schedule
// The reward halves every halvingIntervalDaaScore blocks (approximately 4 years)
func (c *coinbaseManager) CalcBlockSubsidy(stagingArea *model.StagingArea, blockHash *externalapi.DomainHash) (uint64, error) {
	// Genesis block gets special reward
	if blockHash.Equal(c.genesisHash) {
		return c.subsidyGenesisReward, nil
	}
	
	// Get the DAA score (block height equivalent)
	blockDaaScore, err := c.daaBlocksStore.DAAScore(c.databaseContext, stagingArea, blockHash)
	if err != nil {
		return 0, err
	}

	// STOKES: Calculate subsidy using Bitcoin-style halving
	blockSubsidy := c.calcHalvingBlockSubsidy(blockDaaScore)
	return blockSubsidy, nil
}

// STOKES: Bitcoin-style halving calculation
// This function calculates the block subsidy based on the number of halvings that have occurred.
// The subsidy starts at preDeflationaryPhaseBaseSubsidy and halves every halvingIntervalDaaScore blocks.
func (c *coinbaseManager) calcHalvingBlockSubsidy(blockDaaScore uint64) uint64 {
	// Calculate how many halving intervals have passed
	// Each interval is approximately 4 years (126,230,400 seconds at 1 block/second)
	halvings := blockDaaScore / c.halvingIntervalDaaScore
	
	// After 64 halvings, the subsidy becomes 0 (since we're using uint64)
	// This is similar to Bitcoin's behavior
	if halvings >= 64 {
		return 0
	}
	
	// Start with the initial subsidy (50 STKS in sompi)
	subsidy := c.preDeflationaryPhaseBaseSubsidy
	
	// Right shift is equivalent to dividing by 2^halvings
	// This implements the halving: subsidy = initialSubsidy / (2^halvings)
	subsidy >>= halvings
	
	return subsidy
}

// STOKES: Removed Kaspa's smooth decay table and float calculation
// The halving schedule is deterministic and doesn't require lookup tables
// Bitcoin-style halving is simpler: subsidy = initialSubsidy >> halvings
...
// STOKES: Added halvingIntervalDaaScore parameter for Bitcoin-style halving
func New(
	databaseContext model.DBReader,
	...
	return &coinbaseManager{
		...
		halvingIntervalDaaScore:                 halvingIntervalDaaScore,
